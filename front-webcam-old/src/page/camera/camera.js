...
function Camera({ setStateResultData, stateResultData }) {
    ...
    const onResults = (results) => {
        ...
        if (results.poseLandmarks) {
            drawConnectors(canvasCtx, results.poseLandmarks, mediapipePose.POSE_CONNECTIONS, { color: 'aqua', lineWidth: 1.5 });
            drawLandmarks(canvasCtx, results.poseLandmarks, { color: 'gray', lineWidth: 1, fillColor: 'rgb(78,250,27)', radius: '2' });
        }
        ...
        if (landmark) {
            ...
            for (let i = 0; i < 33; i++) {
                if (landmark[i].x < 1 && landmark[i].x > 0 && landmark[i].y < 1 && landmark[i].y > 0)
                    state_pose = true
            }

            if (state_pose === true) {
                const new_calc_data = { pose_data: results, kind_exercise: stateResultData.kind_exercise, state_change_exercise: state_change_exercise }
                setCalcResult(Analysis_exercise(new_calc_data));
            }
            else {
                console.log("Your entire body must be in camera")
    ...
    useEffect(() => {
        ...
        setUnrealVideoUrl(`video/${category}/${exercise}.mp4`)
    ...
    useEffect( () => {
        console.log(max_accuracy)
        if(max_accuracy > 91) setTipSpeaker("Very good, keep it like this")
    ...
    return (
        <>
            <video id='unrealvideo' ref={videoRef} key={unreal_video_key} width='100px' height='100px' controls onEnded={() => {
